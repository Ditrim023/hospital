...
;(function($) {
  ...
  var SlimTable = {
	...
	createTableBody: function() {
		...
		{
			t_obj1 = document.createElement("li");
		    $(t_obj1).addClass("page-item").
				  on('click',{self: this},this.handlePageChange).
			append('<a class="page-link" href="#">'+(l1+1)+'</a>');
	...
	addPaging: function( tblObj ) {
		...
		tObj2 = $("<ul></ul>").
				addClass('pagination');
        $(tObj2).css({'margin-top' : '0','display':'inline','float' : 'right'});
		this.state.btnContainer = tObj2;

		//
		tObj1 = $("<div></div>").
			addClass('pag');
		$(tObj1).css({'width' : '90%','margin-top' : '2px'});
        tObj1.append(tObj2);
